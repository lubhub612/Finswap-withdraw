...
`
  ...
  const getUserWalletBalance = async () => {
    // let url = `https://polygonlive.org/dashboard/api/withdrawal_balance.php?address=${userAddress}`;
    let url = `https://sssworld.live/dashboard/api/usd_balance.php?address=${userAddress}`;
  ...
  const handleUserRegister = async () => {
    ...
    try {
      ...
      if (_waitfortx) {
        let _reg = axios.post(
          `https://sssworld.live/dashboard/api/register.php?refid=123456&address=${userAddress}`
        );
    ...
    try {
      ...
      if (_wait) {
        let depositApi = await axios.get(
          `https://sssworld.live/dashboard/api/fund.php?address=${userAddress}&amount=${depositAmount}`
        );
